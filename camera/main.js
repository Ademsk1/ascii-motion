...
function zoom(mode) {
  let zoomVal
  const { zoom } = track.getConstraints()
  if (mode === 'in') {
    zoomVal = Math.max(zoom + zoomCap.step, zoomCap.max)
  } else if (mode == 'out') (
    zoomVal = Math.min(zoom - zoomCap.step, zoomCap.min)
  )
  track.applyConstraints({ zoom: zoomVal })
}
